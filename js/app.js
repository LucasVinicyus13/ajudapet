...
function populateCityOptions(state, selectedCity = '') {
    const citySelect = document.getElementById('addpet-city');
    ...
    const options = ['<option value="">Selecione uma cidade</option>'];
    cities.forEach((city) => {
    ...
    if (pet) {
        ...
        if (birthInput) {
            if (pet.dataNascimento) {
                ...
                const d = new Date();
                d.setDate(d.getDate() - pet.idadeDias);
                birthInput.value = d.toISOString().slice(0,10);
            } else {
                birthInput.value = '';
            }
        }
        const stateValue = pet.estado || pet.estadoSigla || pet.uf || '';
        const cityValue = typeof pet.cidade === 'string' ? pet.cidade.split(',')[0].trim() : '';
        if (stateInput) {
            stateInput.value = stateValue;
        }
        if (cityInput) {
            populateCityOptions(stateValue, cityValue);
            cityInput.disabled = !stateValue;
        }
...
async function initAddPetForm() {
    ...
    const stateSelect = document.getElementById('addpet-state');
    const citySelect = document.getElementById('addpet-city');
    if (!form || !modal || !categorySelect || !categorySelectedList || !categoryOptionsList || !stateSelect || !citySelect) return;
    ...
    const setCityControl = (selectedCity = '') => {
        ...
        if (!hasState) {
            ...
            populateCityOptions(stateSelect.value, selectedCity);
        }
    };

    stateSelect.addEventListener('change', () => setCityControl());
    setCityControl();
    ...
    form.addEventListener('submit', async (event) => {
        ...
        if (birthdateValue) {
            ...
            idadeDias = Math.max(0, Math.floor(diffMs / (1000 * 60 * 60 * 24)));
            idade = `${idadeDias} dias`;
        }
        const cidade = cityInput.value.trim();
        const estado = stateInput ? stateInput.value.trim() : '';
